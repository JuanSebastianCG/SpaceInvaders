...
//-----------------Constructor----------------------------------------
    ...
    */
    public groupOfInvaders(int xInicial, int yInicial, int row, int column, int speed,int refreshRateInvaders) {
        this.row = row;
        this.column = column;
 ...
 */
    ...
    public void addInvader(int x, int y, int width, int height, int speed, int speedBullet, long refreshShoot) {
        ...
        Invader aux = new Invader(x, y, width, height, speed, speedBullet, refreshShoot);
    ...
    public int searchAlive(){
         int size=0;
        for (Invader invader : invaders) {//busca quien esta vivo
